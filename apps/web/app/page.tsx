...
export default async function Home() {
  ...
  return (
    <main className="overflow-hidden" id="content">
      ...
      <section className="relative bg-[#f5f7f3] py-20 sm:py-24" id="stack">
        <div className="container section-reveal">
          ...
          <div className="service-stage mt-16">
            ...
            <aside className="observability-panel">
              ...
              <div className="mt-7 grid grid-cols-2 gap-4 border-t border-white/10 pt-6"><MiniMetric value="2" label="core services" /><MiniMetric value="4" label="profile groups" /></div>
            </aside>
          </div>
        </div>
      </section>

      <section className="bg-white py-20 sm:py-24" id="cli">
        <div className="container section-reveal grid items-center gap-14 lg:grid-cols-[0.78fr_1.22fr]">
          <div>
            ...
            <div className="mt-9 divide-y divide-border border-y">
              {commands.map(([command, detail]) => <div className="flex items-center gap-4 py-4" key={command}><ChevronRight className="size-4 shrink-0 text-primary" /><code className="w-28 shrink-0 font-bold text-foreground">{command}</code><span className="text-sm text-muted-foreground">{detail}</span></div>)}
            </div>
          </div>
          <TerminalPreview />
        </div>
...
function Principle({icon:Icon,number,title,body}:{icon:LucideIcon;number:string;title:string;body:string}) { return <article className="principle"><span className="principle-number">{number}</span><Icon className="size-5 text-emerald-300" aria-hidden="true" /><h3>{title}</h3><p>{body}</p></article>; }
